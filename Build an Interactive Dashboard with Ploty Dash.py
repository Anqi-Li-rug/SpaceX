...
def get_pie_chart(entered_site):
    if entered_site=='ALL':
        ...
        filtered_df['class']=filtered_df['class']/filtered_df['class'].sum()
        fig=px.pie(filtered_df,values='class',names='Launch Site',title='launch')
        return fig
    else:
        filtered_df=spacex_df[spacex_df['Launch Site']==entered_site]
        df2=filtered_df.groupby(['class'])['Flight Number'].count().reset_index(name="count")
        fig=px.pie(df2,values='count',names='class')
        return fig
# TASK 4:
